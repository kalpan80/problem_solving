...
planning_agent = Agent(
        ...
        instructions="You are a planning agent. Your job is to break down complex tasks into smaller manageable subtasks using sub agents."
                    ...
                    "Please all the agents till accuracy of more than 95% is achieved on test results. "                     
                     "You need to continue one complete iteration, and depending on the results decide on next set of parameters for fine tuning the model"
                     "You need to continue for 10 iterations of complete end to end model training and validation to summarize the results."
...
async def execute():
    result = Runner.run_streamed(starting_agent=planning_agent,
                             input="Start the process of hyperparamater tuning, and generate the results",
                             max_turns=50)
